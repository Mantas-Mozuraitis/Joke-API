...
// GET random joke
app.get("/random", (req,res)=>{res.json(jokes[Math.floor(Math.random() * jokes.length)]);})

// GET a specific joke
app.get("/jokes/:id", (req,res)=>{
    const joke = jokes.find((joke) => joke.id == req.params.id);
    joke?res.json(joke):res.json({ error: 'ID does not exist' });
})

// GET joke filtered by type
app.get("/filter", (req,res)=>{
    const jokesoftype =[];
    jokes.forEach((joke)=>{
        if(joke.jokeType == req.query.type){jokesoftype.push(joke);}
    })
    jokesoftype.length>0?res.json(jokesoftype):res.json({ error: 'No jokes found' }); 
})
...
app.listen(port, (req,res)=>{
    console.log(`API is live and listening on port : ${port}`);
})
